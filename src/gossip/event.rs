...
pub(crate) struct Event<T: NetworkEvent, P: PublicId> {
    ...
    // The hashes of the oldest events by each peer which comprise not-yet-stable blocks this event
    // can see, along with the sorted payloads of all blocks made valid by that event.
    pub valid_blocks_carried: BTreeMap<P, (Hash, BTreeSet<T>)>,
